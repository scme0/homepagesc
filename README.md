...
If there is only one host/path in the ingress all the annotations below can be used as is. If there are multiple host/paths then you will need to set the annotations per host/path. The way this works is by altering the annotations as such: `hompagesc.io/<service_name>_<service_port>_<annotation_label>`
For example, if your path forwards traffic to a service call `my-service` on port `1234` you would update the annotations like to something like this `homepagesc.io/my_service_1234_enable`
